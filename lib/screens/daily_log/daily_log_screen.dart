          ...
          child: InkWell(
            ...
            child: Container(
              ...
              child: Row(children: [
                ...
                Text(isToday ? 'Today — ${formatDate(_selectedDate)}' : formatDate(_selectedDate),
                    style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 15)),
                const Spacer(),
                Text('Change', style: TextStyle(color: AppTheme.primary,
            ...
            : ListView.builder(
                ...
                itemBuilder: (_, i) {
                  ...
                  return _FlockLogCard(
                    flock: flock, log: log, date: _selectedDate,
                    onSave: (eggs, mortality, notes) async {
                      ...
                      if (log != null) {
                        ...
                        log.notes = notes;
                        log.synced = false;
                        await provider.update(log);
                      } else {
                        await provider.add(DailyLog(
                          id: const Uuid().v4(),
                          date: _selectedDate,
                          flockId: flock.id,
                          eggsCollected: eggs,
                      ...
                      if (mortality > 0) {
                        await context.read<FlockProvider>()
                            .addMortality(flock.id, mortality);
                      }
                    },
                    onDelete: log == null
                        ? null
                        : () => context.read<DailyLogProvider>().delete(log),
                  );
                },
              ),
        ),
...
class _FlockLogCard extends StatefulWidget {
  ...
  const _FlockLogCard({
    required this.flock, required this.log, required this.date,
    required this.onSave, this.onDelete,
  });

  @override State<_FlockLogCard> createState() => _FlockLogCardState();
}
...
class _FlockLogCardState extends State<_FlockLogCard> {
  ...
  bool _saving = false, _expanded = false;

  @override
  void initState() {
    super.initState();
    _eggsCtrl      = TextEditingController(text: widget.log?.eggsCollected.toString() ?? '');
    _mortalityCtrl = TextEditingController(text: widget.log?.mortality.toString() ?? '0');
  ...
  Future<void> _save() async {
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('${widget.flock.name} — ${formatEggs(eggs)} logged')));
    }
  }
          ...
          Row(children: [
            CircleAvatar(
              ...
              child: Icon(hasLog ? Icons.check : Icons.egg_outlined,
                  color: hasLog ? AppTheme.primary : Colors.grey.shade400, size: 18),
            ),
            const SizedBox(width: 12),
            ...
            IconButton(
              icon: Icon(_expanded ? Icons.expand_less : Icons.edit_outlined, size: 20),
            ...
            Row(children: [
              Expanded(child: TextField(
                ...
                decoration: const InputDecoration(
                    ...
                    prefixIcon: Icon(Icons.egg_outlined), hintText: '0'),
              )),
              const SizedBox(width: 12),
              Expanded(child: TextField(
                ...
                decoration: const InputDecoration(
                    ...
                    prefixIcon: Icon(Icons.remove_circle_outline), hintText: '0'),
              )),
            ]),
            const SizedBox(height: 12),
            TextField(
  ...
  Widget build(BuildContext context) => Center(
    child: Padding(
      ...
      child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        ...
        const SizedBox(height: 16),
        const Text('No active flocks', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
        const SizedBox(height: 8),
        Text('Add a flock first from Settings → Manage Flocks.',
